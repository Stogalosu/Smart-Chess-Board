<!--
  This journal is auto generated by Hack Club Blueprint.
-->
# for Blueprint

# 6/11/2025 - Made the first version of the board design  

_Time spent: 2.7h_  

4.11.25
I spent about 45 minutes researching possible parts for the project, including stepper motors, electromagnets and main boards. I was initially thinking about using an Arduino board, but I realized that the GUI app that I want to build is very complicated to write in C, so I switched to a Raspberry Pi.

5.11.25
I designed the top of the board in Fusion 360. It was to big to 3d print (on my own printer) in one piece, so I had to break it up into 4 smaller ones. I made a cross-shaped beam with holes to hold them together. That was also too big, so it had to be spit up as well. I chose to connect it all in the middle, but I have some concerns with it being too thin. I hope it will hold up.

Here is the bottom side of the board, without the beam. The top side looks like any other chess board.
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6ODg4MSwicHVyIjoiYmxvYl9pZCJ9fQ==--9e1571e2d0618dfcfb63045cadbc0414aa8a972e/image.png)

This is the beam.
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6ODg4MiwicHVyIjoiYmxvYl9pZCJ9fQ==--a9fdc58782933ef15c53242d76fbdc0c32b2f4e8/image.png)

  

# 10/11/2025 - Tweaked board and created XY-joint  

_Time spent: 4.0h_  

6.11.2025
Instead of pins coming out the beam to hold the board together, I decided to use some stainless steel pins that connect the beam with the pieces and made the necessary adjustments. This took about 30 minutes.

9.11.2025
Added screw holes on the side of the board for attachment to the case. I used M3 screws. This didn't take a lot of time, but I also did some research on the core xy cartesian system, which I plan on using (about 1 hour in total)

10.11.2025
Did some more research on a possible electromagnet, steel rods for the y-axis and linear guiderail for the x-axis and made a xy-joint: a piece which contains a linear bearing that runs along the y-axis and attaches to the guiderail of the x-axis.
I decided to get rid of the bead that holds the board together and just use those steel pins to join the pieces directly, but haven't made the changes.

Here is the modified board:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MTAxNDcsInB1ciI6ImJsb2JfaWQifX0=--5a5e5372c7ef03b753cd8ae964248f347ccf3e50/image.png)

and the xy-joint (it's still and early prototype, I have to split it in two and add screw holes):
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MTAxNDYsInB1ciI6ImJsb2JfaWQifX0=--ad5a8c881b46c59387aa901d553a9db522c8c33d/image.png)
  

# 14/11/2025 - Worked on Core XY System  

_Time spent: 4.0h_  

10.11.2025
I added screw holes to the xy joint. I then had to move them on the other side, because on the top of the joint I will have to mount the idler pulleys. I also gave up on the hexagonal shape, because it was too hard to 3d print.

14.11.2025
I started work on the part that actually moves the chess pieces around. I designed the guiderail carriage  and thought of an idea of how to move a permanent magnet up and down using a servomotor. I started the sketch for the mechanism.

Here is the xy joint seen from below:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MTExNzQsInB1ciI6ImJsb2JfaWQifX0=--ec30525d409c36025d4cc94bba5aaf0a712c995f/image.png)

Here is the sketch for the carriage. I am thinking of a mechanism that uses a disc with a pin to pull a rod that has the magnet at the end up and down.
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MTExNzYsInB1ciI6ImJsb2JfaWQifX0=--e62bb8a6c19dde552b6b55ab5d1837e5e3297d3a/image.png)


And this is the core XY system so far:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MTExNzMsInB1ciI6ImJsb2JfaWQifX0=--05cbc93aa3f2a4e047e9b44ccf86854f4b4011e5/image.png)
  

# 15/11/2025 - Designed the magnet mechanism  

_Time spent: 2.5h_  

15.11.2025
I finished the design of the mechanism I started yesterday. As I said, I went for a piece that holds the magnet and goes up and down, guided by the holes in a bigger piece. It is moved by a disc with a pin.
I 3d printed what I thought was a working version, but, apparently, some tolerances and dimensions had been messed up when I made some edits. I fixed them and I'm going to print the new version tomorrow.

Here is the big piece with the magnet-holding one in it:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MTE1NTYsInB1ciI6ImJsb2JfaWQifX0=--a5e08d5c4d975696e035d204cb4b6f63da36bfee/image.png)

An this is the wheel with the pin. You can see how it interacts with the one with the magnet.
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MTE1NTcsInB1ciI6ImJsb2JfaWQifX0=--12129caca05cf353ae4242482e329efd089732bd/image.png)

  

# 24/11/2025 - Worked on the carriage  

_Time spent: 3.0h_  

16.11.2025
I made some adjustments and printed a version that, with a bit of sanding, was great. I had some concerns about the pin breaking, but that didn't happen, now I have to see if a magnet of the chosen size will be strong enough.

During the week, I added screw holes to mount the mechanism to the guiderail carriage. I had some trouble because the disc was in the way of the screws and I had to use headless screws for that part. I also struggled with the dimensions of the servomotor and didn't make much progress because of it.

24.11.2025
I found a 3d model of the servomotor that allowed me to design the mounting. The problem is that it partly obstructs the hole where the wheel is inserted, So I had to make a bit of space and shrink the wheel. I hope it'll fit.

Here is the mounting without the motor:
![Screenshot 2025-11-24 215010](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MTQwODMsInB1ciI6ImJsb2JfaWQifX0=--45ad92e1c944f2ac413ffdddd0dbd737c484d67d/Screenshot%202025-11-24%20215010.png)

And here it is with it:
![Screenshot 2025-11-24 215016](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MTQwODIsInB1ciI6ImJsb2JfaWQifX0=--896596de6dc899ec311512e1dc4874fe0a6e58a3/Screenshot%202025-11-24%20215016.png)

  

# 4/12/2025 - Added pulleys and belt attachment holes  

_Time spent: 2.9h_  

29.11.2025
I made the first sketches for the core xy system: I placed the pulleys.

30.11.2025
I modeled the pulleys according the the ones I decided on using. I'm using the same ones usually found in 3d printers. I also searched for steel pins to fixate the pulleys and modeled holes for the belts to attach to the carriage. I was inspired by my Prusa 3d printer that has something similar.

3.12.2025
I positioned the pulleys on two levels to allow the two belts to work independently.

Here you can see the holes for the belt:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MTg1ODQsInB1ciI6ImJsb2JfaWQifX0=--5c7a677a7f286531c6e346ade747674b8b33ecc7/image.png)

And here is the whole system, with the pulleys spread on two levels:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MTg1ODUsInB1ciI6ImJsb2JfaWQifX0=--b73253741dd1ce8d2911d013f9262069b4160c80/image.png)

  

# 21/12/2025 - Created mount for stepper motors  

_Time spent: 4.1h_  

Over the past week I've had some challenges with the motors. I realized they were too big to fit in the vertical space that I had arbitrarily chosen. This was not all that bad, as I could just make the whole board taller, but they also didn't fit sideways: the carriage would have hit them when going to the corner and the belt would have had some weird angles. I then thought of methods to make them fit. The idea I came up with was to make the x-axis longer, thus creating space for lowering the pulleys. In my head, this would allow me to fit a slimmer motor upside down, over the pulleys. I did not have the space and these motors were much more powerful and required move power, which I didn't really have any idea how to supply.

I began to worry that I might not be able to work this out, but, after scrapping the slim motor idea, I realized that rotating my current motor would solve the carriage problem. I would just have to accept the board is now about 6 cm tall. I thought this was my only option, so I went for it.

I created the mounts for the motors, as well as what I hope to be a belt tensioning system that uses screws to move the mount and the motor.

Here is the left mount:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MjcxNzAsInB1ciI6ImJsb2JfaWQifX0=--08b584da3fa7fc63d5f74cad446fae342ee8e3f9/image.png)

And here is the right one:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MjcxNzIsInB1ciI6ImJsb2JfaWQifX0=--c6b7d01f49fd8624d0804d339375dea686a426ff/image.png)
  

# 26/12/2025 - Redesigned board and created ejecting mechanism  

_Time spent: 4.2h_  

Since the last entry, I've tweaked the board design. I've removed the connecting beam and the 4 pieces now directly connect using steel pins. I've also extended the lateral walls and added screw holes. After that, I created the mechanism for ejecting pieces off the board, after they are taken by the other player. I chose a special square and I added a hinge to allow it to rotate about 20 degrees in order for the chess piece to slide off the table. I initially had two ideas that are hard to describe, but I ended up adding some prisms to both the special square and the magnet carriage that allow the carriage to rotate the square.

Here is the special square seen from underneath. You can see the prism:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MzAxMTksInB1ciI6ImJsb2JfaWQifX0=--49b769c0ccaf550f9ee0b44d4e51998316bede5f/image.png)

Here is the ramp I added for the sliding piece. You can also see the prism added to the carriage:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MzAxMjAsInB1ciI6ImJsb2JfaWQifX0=--f2acb612103cb7c1890b5768f593c9ed224f8641/image.png)

And here is the whole board:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6MzAxMjEsInB1ciI6ImJsb2JfaWQifX0=--0956036ff5ae234b35f8b82236a81b787a0a8408/image.png)
  

# 17/1/2026 - Created schematic for PCB  

_Time spent: 5.7h_  

27.12-29.12.2025
Beside moving the pieces, I want the board to also detect where the pieces are on the table, so I can also make my moves. For this, I am making a PCB that will go directly under the pieces. It will have hall sensors for every piece, thus being able to detect their presence.
So I started to design the schematic. My dad helped my throughout the process. I decided to use a Raspberry Pi Zero for the board, because the other ones were too big. Because the board requires a 8x8 matrix, we used a I2C expander to reduce the number of pins needed. We also added another chip that provides more current for the sensors. I also thought of integrating connectors for the stepper motors and servomotor that connect to the Raspberry Pi. For connecting the stepper motors, we had to use an 8-bit buffer for logic level change. (conversion from 3V supplied by the pi to 5V required by the motors). 

Because the board is split into 4, the PCB had to be spit as well. Because of this, at the end, we put some thought into the placement of connectors that connect the 8x8 matrix to the Pi. We positioned them so that 3 of the 4 PCBs are identical, to save on cost. The 4th one has all the chips and motor connectors on it, as well as the cutout for the piece eject system.

Here is the schematic, broken into 3 pages:

This is for the 3 identical PCBs
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6ODQ2MjEsInB1ciI6ImJsb2JfaWQifX0=--6907ea10732245c489d93e3017673d45b98dc550/image.png)

This is the special one
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6ODQ2MjIsInB1ciI6ImJsb2JfaWQifX0=--2cbab91ac1d57ee44bd3363c77a9f51f08e8c414/image.png)

And this page is dedicated to the Pi and the connectors for the motors
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6ODQ2MjMsInB1ciI6ImJsb2JfaWQifX0=--c46f7ec74ec42f211edb255b84bb73cd5657eaaa/image.png)  

# 18/1/2026 - Started PCB  

_Time spent: 2.0h_  

29.12.2025
I assigned all of the footprints to all of the parts. Some of them were harder to find. I also searched and settled on some pulleys. I updated the BOM to include them and started designing the PCB. I made a simple box and arranged the sensors according to the dimensions of the board.

Here it is:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6ODQ5NzUsInB1ciI6ImJsb2JfaWQifX0=--69667923765f01b45573a951b87eee5707e256bf/image.png)
  

# 26/1/2026 - Finished the PCB layout  

_Time spent: 4.7h_  

17.01-18.01.2025
I arranged all of the components on the PCB and settled on a design that has some extensions that connect with neighboring PCBs. On the special PCB, I crammed the stepper motor drivers and the Raspberry Pi connectors. I had to overlap them to make them fit, and I will buy a longer header for the Pi.
Before doing the routing, I'm going to continue designing in Fusion.

Here is the normal PCB:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6OTIyNTAsInB1ciI6ImJsb2JfaWQifX0=--891c96fd9abfa9793a1d132030adf87e1794a59e/image.png)

And here is the special one:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6OTIyNTEsInB1ciI6ImJsb2JfaWQifX0=--8e8c018d9e24844e21c5f1e9abd0e556c9d3edcd/image.png)

  

# 28/1/2026 - Starting to wrap up  

_Time spent: 7.5h_  

Over the past week, I've been busy fixing little details and putting the project together. I've made spaces for the components on the PCB, I've searched and imported 3d models for the connectors, Raspberry Pi and drivers. I've also painstakingly extruded all the holes for screws and nuts that will keep the whole thing together and rearranged the pins that connect the 4 quarters of the board.

This is how it looks now:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6OTQxMzUsInB1ciI6ImJsb2JfaWQifX0=--198b47edab9db5c178372f871c485b21b14d6862/image.png)
  

# 30/1/2026 - Adding the finishing touches  

_Time spent: 5.0h_  

29.01.2026
I added the last touches to finish the project: added screw holes in the PCB, created pins that connect the two slices of the board. The PCB goes in between them. I also created a support and a guiding piece for the servomotor cable and added holes for the pi's ports. At the end, I routed the PCB, updated the BOM and gathered all the files for submitting the project.

Here's the finished board:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6OTUwNjUsInB1ciI6ImJsb2JfaWQifX0=--4f0da3e6235e011ec9d69eb07a18fd518de82d1f/image.png)

and the PCBs:
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6OTUwNjYsInB1ciI6ImJsb2JfaWQifX0=--d3b811f29fe89f74911eecf469ef35ce3852ac44/image.png)
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6OTUwNjcsInB1ciI6ImJsb2JfaWQifX0=--200d9ee2f09545146e8bbbf394765617b016980f/image.png)


  

# 4/2/2026 - Fixed PCB holes  

_Time spent: 0.3h_  

04.02.2026
I realized that the hole I had made in the PCB were off by almost a millimeter, so I fixed them. I also remembered that I forgot to add holes for the pins connecting the two plastic pieces of the board "sandwich".

You can see the new holes around the middle of the PCB. The tileable one has more holes, because they always have to be towards the center of the board.
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6OTgxNzYsInB1ciI6ImJsb2JfaWQifX0=--578222cd6163978aad81a539a08a4ff9879fb0d0/image.png)
![image](https://blueprint.hackclub.com/user-attachments/blobs/proxy/eyJfcmFpbHMiOnsiZGF0YSI6OTgxODAsInB1ciI6ImJsb2JfaWQifX0=--e77aa6d6b7463ad0f87a5bd59ce2f57029463ab3/image.png)

# for Horizons

# 25/2/2026 - Adding pin holes to x-axis idlers

_Time spent: 0.88h_

I added holes for the pins that the pulleys go onto, as the model did not include them previously.

[Lapse recording](https://lapse.hackclub.com/timelapse/YqsuZjYVIbya)



# 10/3/2026 - More fixing and editing pieces

_Time spent: 0.25h_

I revisited the pin holes and added a small brim to prevent excessive friction between x-axis idler and pulleys. I also started editing the actual chess pieces by resizing the pawn and its magnet hole.

[Lapse recording](https://lapse.hackclub.com/timelapse/71xHFnRw1OqV)



# 11/3/2026 - Test printing and more piece editing

_Time spent: 1.06h_

Printed some cut pieces of the board to test if tolerances are right. I continued editing the pawn piece by cutting it in two pieces and adding a snap fit to allow the magnet to be inserted after print. Also, I edited X-carrige to avoid interference of belts with other moving pieces.

[Lapse recording](https://lapse.hackclub.com/timelapse/D2Eyl7nXQGMD)



# 13/3/2026 - Editing chess pieces

_Time spent: 1.68h_

I added remaining pin holes while increasing tolerances. Then, I increased the X-carriage belt tolerances and I printed a X-axis idler as well as a X-carriage to test them. I continued editing the other chess pieces in the same way as the pawn.

[Lapse recording](https://lapse.hackclub.com/timelapse/JTLcyxXzvGlR)



# 14/3/2026 - Screw holes and snap fits

_Time spent: 2.08h_

When slicing the bottom parts of the board, I realized that the screw holes were not that pretty, so I reworked them. I finished editing the chess pieces and now have a complete set and each piece can have a magnet inserted into it. Also, I added fillets to the sides of the board to make it prettier.

[Lapse recording](https://lapse.hackclub.com/timelapse/GV2tJu4BBnWR)



# 15/3/2026 - Puzzles and holes

_Time spent: 0.76h_

I (hopefully) strengthened the bottom of the board by adding puzzle-like connectors. I adjusted Raspberry Pi and pin holes according to connector length and adjusted eject mechanism angle after realizing that it would collide with the belts.

[Lapse recording](https://lapse.hackclub.com/timelapse/B2BEkGx1v8uj)



# 16/3/2026 - Belt tensioning

_Time spent: 0.96h_

I worked on the belt tensioning mechanism by making a screw hole in the motor holder and an elongated one on the bottom of the board. This way, the motor can be pulled to stretch the belt and then fixed in place by tightening the screws. I also printed the cable holder for the stepper motor to test it and finally began printing the board by printing the first bottom quarter.

In the second session, I added spaces for pin solders on the back of the PCB and widened spaces for nuts in all pieces.

[Lapse recording 1](https://lapse.hackclub.com/timelapse/-cZruy2iMTlf)
[Lapse recording 2](https://lapse.hackclub.com/timelapse/mS02Os0n7fhu)



# 17/3/2026 - Fixing motor nuts

_Time spent: 0.28h_

I fixed nut holes for the motor holes: the nut was too close to the motor and wouldn't have entered the hole completely.

[Lapse recording](https://lapse.hackclub.com/timelapse/IEAsvMDNfgVc)



# 20/5/2026 - Final check before print

_Time spent: 0.8h_

I checked everything before printing and repaired little mistakes. I again moved the Raspberry Pi pin holes, which, apparently, were not in the right place, I added fillets to the eject mechanism, I added tolerances for the PCB. Then, after everything was right, I exported all of the pieces I had to print.

[Lapse recording](https://lapse.hackclub.com/timelapse/4Uwjsaszz5XT)


# 23/5/2026 - Chamfer and slicing

_Time spent: 0.68h_

I added chamfers to the edges of the board to hopefully make them fit together easier and sliced all top quarters using multiple colors.

[Lapse recording](https://lapse.hackclub.com/timelapse/xWTb6wggGUwi)
